...
class Runner(object):
    ...
    def run(self, scenario_data):
        ...
        if 'fitness' in sim_result:
            return float(sim_result['fitness']), scenario_id
        else:
            return 0.0, scenario_id
    ...
    def _run_scenario(self, scenario_id, scenario_data):
        ...
        record_file = os.path.join(self.record_path, scenario_id + '.obj')
        ...
        resultDic = {}
        try:
            resultDic, record_dic = self.sim.runSimulation(scenario_data, self.scenario_env_json, scenario_id)
        ...
        if os.path.isfile(record_file):
            os.system("rm " + record_file)
        with open(record_file, 'wb') as r_f:
            # global record_dic
            pickle.dump(record_dic, r_f)
